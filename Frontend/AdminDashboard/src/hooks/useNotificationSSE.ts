...
export function useNotificationSSE() {
  ...
  const connect = useCallback(async () => {
    ...
    if (!token) {
      console.log("SSE: No admin token, skipping connection");
      return;
    }
    ...
    console.log("SSE: Connecting to admin notifications stream...");

    try {
      const response = await fetch(`${API_URL}/admin/notifications/stream`, {
        method: "GET",
        headers: {
      ...
      console.log("SSE: Connected to admin notifications stream");
